...
const messageColor = process.env.MESSAGE_COLOR || '#ff8080';
const fs = require('fs');
...
app.get('/messages', function(req, res) {
  ...
  const messagesArr = fileContents.split('\n');
  console.log(`We have a connection, sending: ${messagesArr.join(' ')}`);
  messages(res, messagesArr, 0);
});

function messages(res, messagesArr, pos) {
  let x=0; const y=0;
  const s = messagesArr[pos];
  const width = 6 * s.length;
  const height = 8;
  const board = new Array(height).fill(0).map(() => new Array(width).fill(0));

  [...s].forEach((ch) => {
    const c = cp437.indexOf(ch);
    if (c > 0 && c < 256) {
      for (let i=0; i<6; i++) {
        let line = (i!=5) ? font[(c * 5) + i] : 0x0;
        for (let j=0; j<8; j++) {
          if (line & 0x01 == 1) {
  ...
  res.write('data: ' + JSON.stringify({
    'board': board,
    'textblockStroke': messageColor,
  }) + '\n\n');
  if (pos < messagesArr.length) {
    setTimeout(() => messages(res, messagesArr, pos), 1000);
  } else {
    res.end();
  }
}
...
app.listen(8080, () => console.log('SSE app listening on port 8080!'));
